...
export default function SearchModal({ isOpen, onClose }: SearchModalProps) {
    ...
    const modalContent = (
        ...
        >
            <div
                className={`${posClass} left-0 right-0 bottom-0 z-10000 flex flex-col rounded-t-2xl bg-white dark:bg-[#0f1710] shadow-xl overflow-hidden`}
                style={{
            ...
            <div className="flex items-center justify-between px-4 py-3">
                <span className="text-lg font-semibold text-gray-900 dark:text-white">{t("header.search")}</span>
                <button onClick={onClose} className="p-2 -mr-2 text-gray-600 dark:text-gray-400" aria-label={t("common.close")}>
                    <X className="w-6 h-6" />
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide px-5 py-6">
                ...
                {/* B. 추천 테마 (아이콘 포함) */}
                <section>
                    <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">{t("search.suggestedThemes")}</h3>
                        ...
                        {RECOMMEND_TAGS.map((tag) => {
                            ...
                            >
                                <div className="w-10 h-10 rounded-full bg-white dark:bg-[#1a241b] flex items-center justify-center text-xl shadow-sm">
                                    <img
                                        src={
                                            CATEGORY_ICONS[CONCEPTS[tag.id as keyof typeof CONCEPTS] || label] ||
                                            CATEGORY_ICONS["기타"]
                                        }
                                        alt={label}
