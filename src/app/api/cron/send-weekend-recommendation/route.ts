...
function buildNotificationContent(
    ...
    if (airStatus === "황사") {
        return {
            title: "이번 주말 황사 예보 🌫️",
            body: "이번 주말 황사가 예상돼요. 실내에서 즐기는 데이트 코스를 추천해드릴게요!",
        };
    }
    if (airStatus === "미세먼지") {
        return {
            title: "이번 주말 미세먼지 주의 😷",
            body: "이번 주말 미세먼지가 예상돼요. 카페·문화 공간 실내 데이트는 어떨까요?",
        };
    }
    if (weatherRisk?.rainLikely) {
        return {
            title: "이번 주말 비 소식 ☔",
            body: "이번 주말 비가 올 것 같아요. 분위기 있는 실내 데이트 코스 어떠세요?",
        };
    }
    if (avgSky >= 3.5) {
        return {
            title: "이번 주말 날씨 흐려요 🌥️",
            body: "이번 주말 흐린 날씨가 예상돼요. 감성 넘치는 실내 데이트 코스 추천해드릴게요!",
        };
    }
    if (avgSky >= 2.5) {
        return {
            title: "이번 주말 구름 조금 🌤️",
            body: "이번 주말 구름이 조금 끼지만 나쁘지 않아요. 가볍게 나들이 어떨까요?",
        };
    }
    return {
        title: "이번 주말 날씨 맑아요 ☀️",
        body: "이번 주말 날씨가 좋을 것 같아요! 야외 데이트 코스 추천해드릴게요 💚",
    };
}
